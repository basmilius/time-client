...
export function loadAvatarClothingConfig(config, loader)
{
	return new Promise(resolve =>
	{
		...
		if (isInitializerLoader)
		{
			loader
				.add(configUrl)
				.on("complete", () => LOADED_CONFIGS[config] = application.getResource(configUrl).data);
...
export class Avatar extends PIXI.Graphics
{
	...
	constructor(figure = "lg-3116-110-92.hr-3163-61.sh-3275-92.hd-195-4.ha-3268-1415-92.ch-255-92", geometry = "vertical", avatarSet = "full")
	{
		...
		this.on("pointerover", () => this.addAction("Wave"));
		this.on("pointerout", () => this.removeAction("Wave"));
		this.on("pointerdown", () => this.addAction("Move"));
	...
	getLibraries()
	{
		...
		this._parts.forEach((settype, type) =>
		{
			...
			{
				const lib = getFigureMap().querySelector(`part[id="${part.id}"][type="${type}"]`).parentNode;
